...
////////////////////////////////////
static int div_printPCIndex = 0;
static md_addr_t div_printPC[256];
static int div_squash_per_mis = 0;
...
/*Divya*/
static counter_t div_squash_cnt = 0;
static counter_t div_num_mispredbr = 0;
...
/////////////////added for project
static struct div_IntegrationTable
{
 struct div_IntegrationTable *next;
 md_addr_t ITpc;
 int div_inames[3]; //get macro MAX_IDEPS
 int div_onames[2]; //get macro MAX_ODEPS
};

static struct div_IntegrationTable *div_IT;
...
///////////////////////////
stat_reg_counter(sdb, "div_squash_cnt",
		   "total number of instructions squashed",
		   &div_squash_cnt, 0, NULL);
  
stat_reg_counter(sdb, "div_num_mispredbr",
		   "total number of branches mispredicted",
		   &div_num_mispredbr, 0, NULL);
/////////////////////////
...
//////////////////////////////
static void div_IT_init(int nlinks);
/////////////////////////////
...
/////////////////////////////////divya palak
  int inames[MAX_IDEPS]; // names of input registers
  /////////////////////////////
...
///////////////////////////////
//#define ITLINK_NULL_DATA		{ NULL, NULL}
//static struct div_IT_link ITLINK_NULL = ITLINK_NULL_DATA;
////////////////////////////////
...
///////////////////////////////////
/* create and initialize an IT link */
#define ITLINK_INIT(ITL, IT)						\
  ((ITL).next = NULL, (ITL).Itab = (IT))
////////////////////////////////////

/* non-zero if RS link is NULL */
#define RSLINK_IS_NULL(LINK)            ((LINK)->rs == NULL)
/////////////////////////////////////
/* non-zero if IT link is NULL */
#define ITLINK_IS_NULL(LINK)            ((LINK)->Itab == NULL)
//////////////////////////////////
...
///////////////////////////////////
/* extra Integration Table pointer */
#define ITLINK_IT(LINK)                 ((LINK)->Itab)
////////////////////////////////////
...
/////////////////////////////////div palak
/* get a new IT link record */
#define ITLINK_NEW(ITDST, myIT)						\
  { struct div_IT_link *n_link;						\
    if (!IT_freeList)						\
      panic("out of IT links");						\
    n_link = IT_freeList;						\
    IT_freeList = IT_freeList->next;				\
    n_link->next = NULL;						\
    n_link->Itab = (myIT); 			\
    (ITDST) = n_link;							\
  }

/* free an IT link record */
#define ITLINK_FREE(LINK)						\
  {  struct div_IT_link *f_link = (LINK);					\
     f_link->Itab = NULL; 				\
     f_link->next = IT_freeList;					\
     IT_freeList = f_link;						\
  }
...
/* FIXME: could this be faster!!! */
/* free an RS link list */
#define ITLINK_FREE_LIST(LINK)						\
  {  struct div_IT_link *fl_link, *fl_link_next;				\
     for (fl_link=(LINK); fl_link; fl_link=fl_link_next)		\
       {								\
	    fl_link_next = fl_link->next;					\
	    ITLINK_FREE(fl_link);						\
...
///////////////////////////////////////
struct div_IT_link {
 struct div_IT_link *next;
 struct div_IntegrationTable *Itab;
}; 

static struct div_IT_link *IT_freeList;

/* initialize integration table */
static void
div_IT_init(int nlinks)			/* total number of entries available */
{
  int i, z;
  struct div_IntegrationTable *IT_link;
  IT_freeList = NULL;
  for (i=0; i<nlinks; i++)
    {
      IT_link = calloc(1, sizeof(struct div_IT_link));
      if (!IT_link)
	fatal("out of virtual memory");
      IT_link->next = IT_freeList;
      IT_freeList = IT_link;
    }
}
/////////////////////////////
...
static void
ruu_recover(int branch_index, struct RUU_station *rs, struct div_IntegrationTable *div_currentIT)			/* index of mis-pred branch */
{
  int i, RUU_index = RUU_tail, LSQ_index = LSQ_tail;
  int RUU_prev_tail = RUU_tail, LSQ_prev_tail = LSQ_tail;
///////////////
   struct div_IT_link *div_IT_new = NULL;
...
/////////////////////////////////////div_printPC[div_printPCIndex] = 111111111;
...
div_currentIT->ITpc = RUU[RUU_index].PC;
div_currentIT->div_inames[0] = RUU[RUU_index].inames[0];
div_currentIT->ITpc = RUU[RUU_index].PC;
div_currentIT->ITpc = RUU[RUU_index].PC;

ITLINK_NEW(div_IT_new, div_currentIT);

if(div_num_mispredbr == 15){
		div_squash_per_mis++;
		md_addr_t myPC = RUU[RUU_index].PC;
		fprintf(stderr, "\nPC of squashed instruction %lld \n", myPC);
		//fprintf(stderr, "nextPC of squashed instruction %lld \n", RUU[RUU_index].next_PC);
		//fprintf(stderr, "tag of squashed instruction %d \n", RUU[RUU_index].tag);
		//fprintf(stderr, "spec mode of squashed instruction %d \n", RUU[RUU_index].spec_mode);
		fprintf(stderr, "status:queued/issued/completed %d %d %d \n", RUU[RUU_index].queued, RUU[RUU_index].issued, RUU[RUU_index].completed);
		fprintf(stderr, "output logical name of squashed instruction %d %d \n", RUU[RUU_index].onames[0], RUU[RUU_index].onames[1]);
		fprintf(stderr, "input operands of squashed instruction %d %d %d \n", RUU[RUU_index].inames[0], RUU[RUU_index].inames[1], RUU[RUU_index].inames[2]);
		//fprintf(stderr, "idep ready of squashed instruction %d %d %d \n", RUU[RUU_index].idep_ready[0], RUU[RUU_index].idep_ready[1], RUU[RUU_index].idep_ready[2]);
	}
//////////////////////////
...
//palak prints here
if(div_num_mispredbr == 15)
	{fprintf(stderr,"\nnumber of squashes for 15th branch = %d\n", div_squash_per_mis);}
...
ruu_writeback(void)
{
  int i;
  struct RUU_station *rs;
  struct div_IntegrationTable *div_currIT;
  ...
  while ((rs = eventq_next_event()))
	...
	if (rs->in_LSQ)
	  ...
	  ruu_recover(rs - RUU, rs, div_currIT);
	  tracer_recover();
...
static struct div_IT_link IT_last_op; // = ITLINK_NULL_DATA;
...
ruu_dispatch(void)
{
  ...
  //////////////////////////////////////////////////////////
  struct div_IntegrationTable *it;
  //////////////////////////////////////////////////////////
